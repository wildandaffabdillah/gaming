...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE products SET name='$name', category='$category', price='$price', description='$description', discount=$discount WHERE id='$product_id' AND seller_id='$seller_id'";
    ...
    if (mysqli_query($conn, $sql)) {
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] == UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
                ...
                $delete_old_sql = "DELETE FROM product_images WHERE product_id=$product_id";
                        ...
                        <form method="post" action="" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" name="category" id="category" required>
                                    <option value="computer" <?php if ($row['category'] == 'computer') echo 'selected'; ?>>Computer</option>
                                    <option value="gaming_chair" <?php if ($row['category'] == 'gaming_chair') echo 'selected'; ?>>Gaming Chair</option>
                                    <option value="vga" <?php if ($row['category'] == 'vga') echo 'selected'; ?>>VGA</option>
                                    <option value="cpu" <?php if ($row['category'] == 'cpu') echo 'selected'; ?>>CPU</option>
                                    <option value="laptop" <?php if ($row['category'] == 'laptop') echo 'selected'; ?>>Laptop</option>
                                    <option value="monitor" <?php if ($row['category'] == 'monitor') echo 'selected'; ?>>Monitor</option>
                                    <option value="other" <?php if ($row['category'] == 'other') echo 'selected'; ?>>Other</option>
                                </select>
                            </div>
                            <div class="mb-3">
                                <label for="price" class="form-label">Price (Rp)</label>
                                <input type="number" class="form-control" name="price" id="price" step="0.01" value="<?php echo $row['price']; ?>" required>
                            </div>
                            <div class="mb-3">
                                <label for="discount" class="form-label">Discount (%)</label>
                                <input type="number" class="form-control" name="discount" id="discount" min="0" max="100" value="<?php echo $row['discount']; ?>">
                            </div>
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <?php if ($current_image) { ?>
                                    <div class="mb-2">
                                        <img src="uploads/<?php echo $current_image; ?>" alt="Current Image" style="max-width: 100px;">
                                    </div>
                                <?php } ?>
